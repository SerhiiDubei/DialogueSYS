...
func next(next_id: String) -> void:
	# DEBUG: показуємо перехід (але НЕ міняємо кольори - це робить passed_title)
	print("🔄 Перехід до next_id: '", next_id, "'")
	
	# Кольори тепер змінюються ТІЛЬКИ через сигнал passed_title
	# НЕ міняємо dialogue_type тут, бо next_id - це internal ID, а не назва title
